...
// The Java class will be hosted at the URI path "/recycle_records"
@Path("/calendar/recycle")
public class calendarRecycleResource {
    ...
    public List<recycle_records> getIt(@PathParam("UserName") String UserName, @PathParam("MMYYYY")String MMYYYY, String authorizationCode) {
    	...
    	if(authenticated)
    	{
    		...
    		try {
				List<recycle_records> recycleList = getDao().queryForEq("UserName", UserName);
				List<recycle_records> monthList = new ArrayList<recycle_records>();
